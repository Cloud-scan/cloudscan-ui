...
└────────────────┬─────────────────────────────────┘
                 ...
                 ▼
┌──────────────────────────────────────────────────┐
│          React UI (This Service)                 │
│          Served by: NGINX / Spring Boot          │
...
│  Port: 8080      │  │ Port: 9090         │
│  - Auth/RBAC     │  │ - Live scan logs   │
│  - Rate limiting │  │ - Status updates   │
│  - Routing       │  │ - Progress stream  │
└────────┬─────────┘  └────────────────────┘
         │
         │ gRPC
         ▼
...
│  Backend Services                           │
│  ┌──────────────┐  ┌─────────────────────┐ │
│  │ Orchestrator │  │  Storage Service    │ │
│  │ - Scans      │  │  - Upload source    │ │
│  │ - Findings   │  │  - Download results │ │
│  └──────────────┘  └─────────────────────┘ │
└─────────────────────────────────────────────┘
```
...
2. **Create Scan:**
   - React → Storage Service: Upload source code
   - React → API Gateway → Orchestrator: CreateScan(artifact_id)
   - Orchestrator → K8s: Spawn runner job

3. **Real-time Updates:**
   - React ← WebSocket Service ← K8s: Stream pod logs
   - React polls Orchestrator for status updates (fallback)

4. **View Results:**
   - React → API Gateway → Orchestrator: GetFindings()
   - Display findings in table with filtering
